...
.calculando_proporcao {
	...
	-webkit-box-shadow: 0 0 10px 1px #f1f1f1;
...
.calculando_proporcao .tabela_desconto_column p { font-size: 13px !important; padding: 1px; margin: 1px; font-weight: normal; width: 100%;}
...
<div class="calculando_proporcao" onmouseover="myFunction_calcular_proporcao()" onclick="myFunction_calcular_proporcao()">
		...
		<div onclick="mostrar_informacoes_cauculo()" style="display:block; clear:both; width: 5%;    margin: auto; float: right; text-align:right;   color: #ccc; background: #ffffff; opacity: 0.2;"><i class="fa fa-info-circle" aria-hidden="true"></i></div>
		<script>
		...
		<div id="informacoes_cauculo" style="display:none;">
			...
			<div class="box_calculos box_investimento">
				...
				<p>Preço Bobina:<br/><input type="number" id="input_preco_bobina" onmouseout="myFunction_calcular_proporcao()"  onmouseover="myFunction_calcular_proporcao()" onkeyup="myFunction_calcular_proporcao()" value="590"></p>
				<p>Preço tinta:<br/>	<input type="number" id="input_preco_tinta" onmouseout="myFunction_calcular_proporcao()" onmouseover="myFunction_calcular_proporcao()" onkeyup="myFunction_calcular_proporcao()" value="685"></p>
			</div>	
...
function myFunction_calcular_proporcao() {
 ...
 // $total_venda = $custo_final * 160; // total de custo x
	  ...
	  if (area_variavel <= 0.0095) {
		$total_venda = 0.51;
	  } else   
	  if (area_variavel <= 0.0105) {
		$total_venda = 0.52;
	  } else   
	  if (area_variavel <= 0.0110) {
		$total_venda = 0.53;
	  } else   
	  if (area_variavel <= 0.0115) {
		$total_venda = 0.54;
	  } else   
	  if (area_variavel <= 0.0120) {
		$total_venda = 0.57;
	  } else    
	  if (area_variavel <= 0.0120) {
		$total_venda = 0.60;
	  } else    
	  if (area_variavel <= 0.0125) {
		$total_venda = 0.63;
	  } else    
	  if (area_variavel <= 0.0130) {
		$total_venda = 0.66;
	  } else    
	  if (area_variavel <= 0.0135) {
		$total_venda = 0.69;
	  } else   
	  if (area_variavel <= 0.0140) {
		$total_venda = 0.72;
	  } else 
...
// alex altera nome do input do plugin de preco de type="number" para type="text"// 
document.getElementById("ywcnp_suggest_price_single").setAttribute("type", "text");
